"""HydroTFT -- CAMELS dataset classes with multi-horizon targets and engineered features.
...
from .datautils import (load_attributes, load_discharge, load_forcing,
                        normalize_features, normalize_starter_features,
                        reshape_data, compute_starter_features)
...
class CamelsTXT(Dataset):
    ...
    def __init__(self,
        ...
        self.use_starter_features = use_starter_features
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Compute starter features on FULL data BEFORE slicing so that
        # rolling windows (e.g. 90-day sum) are complete at all timesteps.
        if self.use_starter_features:
            extra_df, _ = compute_starter_features(df)

        # we use (seq_len) time steps before start for warmup
        start_date = self.dates[0] - pd.DateOffset(days=self.seq_length - 1)
        # For forecast mode, we need extra days after the end date for targets
        extra_target_days = max(self.pred_days - 1, 0)  # nowcast (0) needs 0 extra
        end_date = self.dates[1] + pd.DateOffset(days=extra_target_days)
        ...
        x = normalize_features(base_x, variable='inputs')

        if self.use_starter_features:
            # Slice starter features to match the date range, then normalize
            extra_x = extra_df.loc[df.index].values
            extra_x = normalize_starter_features(extra_x)
            x = np.concatenate([x, extra_x], axis=1)

        y = np.array([df['QObs(mm/d)'].values]).T

        x, y = reshape_data(x, y, self.seq_length, self.pred_days)

        if self.is_train:
            # Deletes all records, where no discharge was measured (-999)
            bad_rows = np.unique(np.argwhere(y < 0)[:, 0])
